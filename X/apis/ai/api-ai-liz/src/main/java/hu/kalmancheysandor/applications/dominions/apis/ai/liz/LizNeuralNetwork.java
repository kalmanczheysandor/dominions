...
public class LizNeuralNetwork extends TNeuralNetwork {
    ...
    public NeuralNetworkTrainingResult train(List<INeuralInputData> trainingDataList, INeuralNetworkTrainingConfiguration trainingConfiguration, List<INeuralInputData> testingDataList) {
        ...
        System.out.println("<<<< LizNeuralNetwork: Training >>>>");
...
//        printDataSet(trainingSet);
        ...
        learningRule.setLearningRate(lizTrainingConfiguration.getLearningRate());
        network.setLearningRule(learningRule);


        // Initialise neural network and train it
        List<NeuralNetworkTrainingResult.Snapshot> trainingSnapshotsList = new ArrayList<>();
        boolean isSaved = false;
        NeuralNetwork<?> bestNetwork = null;
        NeuralNetworkEvaluationResult evaluationResult;
        double bestPrecisionRate = 0.0;
        for (int turn = 1; turn <= lizTrainingConfiguration.getMaxTurn(); turn++) {
            System.out.println("++++++++TURN:" + turn + "+++++++++++++++++++++++++++++++++++++++++++");
            network.learn(trainingSet);

            // Evaluate current phase
            evaluationResult = evaluationOfANetwork(network, testingDataList);
            trainingSnapshotsList.add(new NeuralNetworkTrainingResult.Snapshot(turn, evaluationResult, lizTrainingConfiguration.getMaxIterationsPerTurn()));
            System.out.println("Evaluation result: " + evaluationResult.getPrecisionRate());

            // Save network-instance if precision is better then latest
            if (evaluationResult.getPrecisionRate() > bestPrecisionRate) {
                bestPrecisionRate = evaluationResult.getPrecisionRate();
                network.save(networkFilePath);
                isSaved = true;
                System.out.println("Better: Neural network saved to '" + networkFilePath + "'");
            }
        }
        ...
        if (isSaved) {
            System.out.println("The very best is re-loaded!");
            network = NeuralNetwork.createFromFile(networkFilePath);
            evaluationResult = evaluationOfANetwork(network, testingDataList);
            System.out.println("The best is evaluated:" + evaluationResult.getPrecisionRate());
        }

        // Generate output
        return NeuralNetworkTrainingResult.builder()
                .precisionRate(bestPrecisionRate)
                .snapshotsList(trainingSnapshotsList)
                .build();
    }
