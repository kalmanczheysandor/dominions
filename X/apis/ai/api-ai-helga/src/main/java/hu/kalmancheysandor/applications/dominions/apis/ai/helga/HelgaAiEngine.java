...
public class HelgaAiEngine extends THelgaAIEngine {


    public HelgaAiEngine() {
        super();
    }
    ...
    public AiDecisionResult makeDecision(AiDecisionContext context) {
        // Initialisation
        BranchData branchData = chooseTheBestStep(context.getGameState(), context.getYourPlayerKey(), 2, 0, context.getPlayers());
    ...
    private BranchData chooseTheBestStep(GameState gameState, int supportedPlayerKey, int maxDeepness, int parentLevelDeepness, Map<Integer, AiDecisionContext.Player> players) {
        int currentLevel = parentLevelDeepness + 1;
        System.out.println("########## LEVEL " + currentLevel + " ####################");

        if (gameState.isFinalState()) {
            System.out.println("FinalStateInput");
        }
        ...
        // Step 1
        Map<Integer, List<Map<Integer, Integer>>> combinationsGroups = generateCombinationGroups(gameState, supportedPlayerKey, players);
        ...
        for (Map.Entry<Integer, List<GameState>> entry : gameStateListGroups.entrySet()) {
            ...
            for (GameState gameStataVariant : gameStateVariants) {
                ...
                if (gameStataVariant.isFinalState()) {
                    System.out.println("FinalStateVariant");
                }

                if (maxDeepness > currentLevel && !gameStataVariant.isFinalState()) {
                    BranchData b = chooseTheBestStep(gameStataVariant, supportedPlayerKey, maxDeepness, currentLevel, players);
                    h = b.getHeuristicValue();
                    heuristicDataList.add(h);
                } else if (maxDeepness == currentLevel || gameStataVariant.isFinalState()) {
...
//       
            ...
            if (worst == null) {
                System.out.println("heuristicDataList:" + heuristicDataList);
                System.out.println("heuristicDataList:" + heuristicDataList.size());
                System.out.println("gameStateVariants:" + gameStateVariants.size());
            }
