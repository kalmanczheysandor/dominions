...
import hu.kalmancheysandor.applications.dominions.apis.server.game.common.proxy.game.*;
...
//    private PlayOrchestrator playOrchestrator;
    ...
    @Autowired
    private EgonAiServerProxy egonAiServerProxy;

    @Autowired
    private LizAiServerProxy lizAiServerProxy;

    @Autowired
    private HugoAiServerProxy hugoAiServerProxy;

    @Autowired
    private HelgaAiServerProxy helgaAiServerProxy;
...
//        GameMap gameMapObj = GameMap.createByJsonFile("maps/map26-H1A1-LizDecisionMaking-001.json");
    ...
    public List<GamePlayItemResponse> listAllRecruitingGamePlay() {
        ...
        for (GameSession gameSession : gameSessionList) {
            ...
            responseItem.setTitle("World map");// TODO: A temporary solution
            responseItem.setDifficulty("Beginner");// TODO: A temporary solution
            responseItem.setFreeSlotCount(gameSession.getFreeSlotCount());
            responseItem.setPlayerAiCount(3);   // TODO: A temporary solution
            responseItem.setPlayerNormalCount(2); // TODO: A temporary solution
            responseItem.setPlayerCurrentCount(1); // TODO: A temporary solution
...
//        }
    ...
    private AiDecisionResponse waitingAiResponse(ArtificialPlayer artificialPlayer, AiDecisionRequest aiDecisionRequestObj) {
        ...
        if (engineType == GameMapEngineType.AI_OTTO) {
            aiDecisionResponseObj = ottoAiServerProxy.generateResponse(aiDecisionRequestObj);
            ...
            aiDecisionResponseObj = lizAiServerProxy.generateResponse(aiDecisionRequestObj);
        } else if (GameMapEngineType.AI_HUGO == engineType) {
            aiDecisionResponseObj = hugoAiServerProxy.generateResponse(aiDecisionRequestObj);
        } else if (GameMapEngineType.AI_HELGA == engineType) {
            aiDecisionResponseObj = helgaAiServerProxy.generateResponse(aiDecisionRequestObj);
        } else {
