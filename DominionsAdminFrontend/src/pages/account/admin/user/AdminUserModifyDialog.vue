<template>
    <TDialog ref="Dialog" title="Modify Admin-User" :closeable="true" @whenDialogClosed="whenParentDialogClosed">
                ...
                <v-tabs-window-item value="Tab1">
                    <v-row>
                        ...
                        <v-col >
                            ...
                            <v-row>
                                <v-col>
                                    <v-checkbox v-model="Form.Fields.FinalisedCheckInput.isChecked"
                                                label="Finalised"
                                                variant="outlined"
                                                density="compact"
                                                clearable
                                ...
                                <v-col>
                                    <v-checkbox v-model="Form.Fields.EnabledCheckInput.isChecked"
                                                label="Enabled"
                                                variant="outlined"
                                                density="compact"
                                                clearable
                    ...
                    <v-text-field v-model="Form.Fields.NameTextInput.value"
                                  label="Name"
...
import adminUserService from "@/services/account/admin/user/AdminUserService";
    ...
    methods: {
        ...
        async eventClickOnModifyButton() {
            try {
                ...
                // Save form value
                await adminUserService.modify(this.uuid, {
        ...
        async open(uuid) {
            try {
                ...
                // Checking permission
                authService.assertEditActionGrantedOn("Account.Admin.User");

                // Retrieve: primary record
                const primaryResult = await adminUserService.accessByUuid(uuid);     // If no primary record is found at uuid than exception is thrown

                // Retrieve: meta record(s)
                const permissionGroupResult = await adminUserService.atEditListPermissionGroup(uuid);

                console.log("Open dialog");
                console.log("Backend.AllPermissionGroups: ", permissionGroupResult.data);
                console.log("Backend.SavedPermissionGroups: ", primaryResult.data.permissionGroups);
                ...
                this.Form.Fields.FinalisedCheckInput.isChecked = primaryResult.data.finalised;
