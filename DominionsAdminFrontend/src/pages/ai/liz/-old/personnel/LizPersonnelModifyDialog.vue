<template>
    <TDialog ref="Dialog" title="Modify lizPersonnel" :closeable="true" @whenDialogClosed="whenParentDialogClosed">
    ...
    methods: {
        ...
        async eventClickOnModifyButton() {
            try {
                ...
                // Save form value
                await lizPersonnelService.modify(this.uuid, {
        ...
        async open(uuid) {
            try {
                ...
                // Checking permission
                authService.assertEditActionGrantedOn("Ai.Liz.Personnel");

                // Retrieve: primary record
                const primaryResult = await lizPersonnelService.accessByUuid(uuid);     // If no primary record is found at uuid than exception is thrown
