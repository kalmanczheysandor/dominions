<template>
    <TDialog ref="Dialog" title="New lizCharacter" :closeable="true" @whenDialogClosed="whenParentDialogClosed">
    ...
    methods: {
        async eventClickOnSaveButton() {
            try {
                ...
                // Save form value
                await lizCharacterService.save({
        ...
        async open() {
            try {
                // Checking permission
                authService.assertAddActionGrantedOn("Ai.Liz.Character");

                // Retrieve: meta record(s)
                const variantResult = await lizCharacterService.atAddListVariant();
