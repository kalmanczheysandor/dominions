...
export default {
    ...
    methods: {
        ...
        async eventResign(actionData) {
            // Input checking
            TParameterValidation.assertParameterIsNotNull(actionData, 'actionData');

            try {
                // Communicate with backend
                const gameReserveResponse = await gameService.sendResignAction({
                    gameSessionUuid: this.GlobalStore.GamePlayPage.gameSessionUuid,
                    endpointKey: authService.getCurrentSessionId(),
                    userUuid: authService.getCurrentUserUuid(),
                    playerIndex: actionData.playerIndex
        ...
        whenActionIsResign(data) {
            this.eventResign(data);
        },

        whenNavigationMenuAttemptsResign(data) {
            this.$refs.GameBoardComponent.eventAttemptResign();
        }
