<template>
    <TDialog ref="Dialog" title="New permissionGroup" :closeable="true" @whenDialogClosed="whenParentDialogClosed">
        ...
        methods: {
            async eventClickOnSaveButton() {
                try {
                    ...
                    // Save form value
                    const result = await permissionGroupService.save({
            ...
            async open() {
                try {
                    // Checking permission
                    authService.assertAddActionGrantedOn("Account.PermissionGroup");
