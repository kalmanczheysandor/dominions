<template>
        ...
        <div id="PlayersPanel">
            <table>
                <tr v-for="(playerItem, playerIndex) in state.players" :key="'PlayersPanel-Row-'+playerIndex">
                    <td>
                        {{playerIndex == yourIndex ? playerItem.name + ' ' + ' (You)' : playerItem.name}}
                    </td>
                    <td>{{configuration.players[playerIndex].type}}</td>
                    <td>{{playerItem.reserveSize}}</td>
                    <td>{{getCellCount + ' / ' + playerItem.occupiedCellCount}}</td>
                    <td>{{playerItem.isIntentionGiven == true ? 'Given' : 'No'}}</td>
                </tr>
            </table>
                    ...
                    <g transform="scale(1.0,1.0)">
                        ...
                        <g transform="translate(+250,0)">
                            <g transform="scale(0.8,0.8)">
                                ...
                                <!--COUNTRIES-->
                                <polygon v-for="(countryItem, countryIndex) in configuration.view.foreground.countries" :key="'Country-'+countryIndex" ffw-type="Country" :ffw-country-key="countryItem.countryKey" :class="countryItem.classes" :points="countryItem.points" :ref="'Country-' + countryItem.countryKey" @click="eventCountryIsChosen(countryItem.countryKey)"/>
    ...
    <div id="IntentionWaitingLayer" v-if="isIntentionGiven">
        <div id="BackDrop"></div>
        <div class="TDialog IntentionWaitingDialog">
            <div class="Body">
                <v-progress-linear color="light-blue" height="2" striped indeterminate rounded></v-progress-linear>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async loadPlayState(initialisationData) {
                ...
                this.maxHumanPlayerCount = initialisationData.maxHumanPlayerCount;
                this.currentHumanPlayerCount = initialisationData.currentHumanPlayerCount;
                this.yourIndex = initialisationData.yourIndex;
                this.turn = initialisationData.turn;
            ...
            countOccupiedCells(playerIndex) {
                ...
                Object.entries(this.state.cells).forEach((cellItem) => {
                    if(cellItem.occupierKey === playerIndex) {
                        count++;
                    }
                });
                return count;
...
<style scoped>
    ...
    #IntentionWaitingLayer > #BackDrop {
        position: absolute;
        top: 0px;
        left: 0px;
        right: 0px;
    ...
    #IntentionWaitingLayer .IntentionWaitingDialog {
        background-color: #f2f2f2;
    }
