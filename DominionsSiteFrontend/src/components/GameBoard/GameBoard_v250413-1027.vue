<template>
    ...
    <div id="PlayersPanel">
        <table>
            <tr v-for="(playerItem, playerIndex) in modelValue.state.players" :key="'PlayersPanel-Row-'+playerIndex">
            ...
            <g transform="translate(0,0)">
                <g transform="scale(1.0,1.0)">
                    ...
                    <g transform="translate(0,0)">
                        <g transform="scale(1.0,1.0)">
                            <!--MERIDIAN LINES-->
                            <line v-for="(meridianItem, meridianIndex) in modelValue.view.background.meridians" :key="'Background-MeridianLine-'+meridianIndex" :x1="meridianItem.x1" :y1="meridianItem.y1" :x2="meridianItem.x2" :y2="meridianItem.y2" style="stroke:#0066ff;stroke-width:1"/>
                            <!--PARALLEL LINES-->
                            <line v-for="(parallelItem, parallelIndex) in modelValue.view.background.parallels" :key="'Background-ParallelsLine-'+parallelIndex" :x1="parallelItem.x1" :y1="parallelItem.y1" :x2="parallelItem.x2" :y2="parallelItem.y2" style="stroke:#0066ff;stroke-width:1"/>
                        </g>
                    </g>
                    ...
                    <g transform="translate(+250,0)">
                        <g transform="scale(0.8,0.8)">
                            <!-- CONTINENT OUTLINE -->
                            <polygon v-for="(continentItem, continentIndex) in modelValue.view.foreground.continents " :key="'ContinentOutline-'+continentIndex" class="Continent" :points="continentItem.points"/>

                            <!--LINKS-->
                            <line v-for="(lineItem, lineIndex) in modelValue.view.foreground.jumpLines" :key="'Line-'+lineIndex" :x1="lineItem.x1" :y1="lineItem.y1" :x2="lineItem.x2" :y2="lineItem.y2" stroke="#003d99" stroke-width="5" stroke-dasharray="5"/>

                            <!--COUNTRIES-->
                            <polygon v-for="(countryItem, countryIndex) in modelValue.view.foreground.countries" :key="'Country-'+countryIndex" ffw-type="Country" :ffw-country-key="countryItem.countryKey" :class="countryItem.classes" :points="countryItem.points" :ref="'Country-' + countryItem.countryKey" @click="eventCountryIsChosen(countryItem.countryKey)"/>
                            ...
                            <!--COUNTRY LABELS-->
                            <text v-for="(countryLabelItem, countryLabelIndex) in modelValue.view.foreground.countryLabels" :key="'CountryLabel-'+countryLabelIndex" ffw-type="CountryLabel" :ffw-country-key="countryLabelItem.ownerCountryKey" :x="countryLabelItem.x" :y="countryLabelItem.y" class="CountryLabel" :ref="'CountryLabel-' + countryLabelItem.labelKey">
    ...
    <div id="PendingLayer">
        ...
        <div class="TDialog PendingDialog">
            ...
            <div class="Footer">
                <button type="button" class="TButton OkButton" @click="eventClickOnLeaveTheGameButton">Leave the game</button>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            getCellCount() {
                return Object.keys(this.modelValue.state.cells).length
            }
        },
        data() {
            return {
        ...
        methods: {
            ...
            eventAttackAction() {
                ...
                if(playerData.reserveSize<troopsSize) {
                    this.$emit('whenTroopSizeIsOverTheAvailable',{expectedTroopSize:troopsSize,reserveSize:playerData.reserveSize});
                }
            ...
            adjustCountryLabels() {
                for(const labelItem of this.modelValue.view.foreground.countryLabels) {
            ...
            adjustCountryLabelPosition(labelItem) {
                ...
                const countryItem = this.modelValue.view.foreground.countries.find(country => country.countryKey === labelItem.ownerCountryKey)
            ...
            countOccupiedCells(playerIndex) {
                ...
                Object.entries(this.modelValue.state.cells).forEach((cellItem) => {
            ...
            refreshBoardState() {
                ...
                Object.entries(this.modelValue.state.players).forEach(([playerIndex, playerItem]) => {
            ...
            refreshAllCountryState() {
                Object.entries(this.modelValue.state.cells).forEach(([cellIndex, cellItem]) => {
            ...
            determineCountryColor(countryIndex) {
                const cellItem = this.modelValue.state.cells[countryIndex];
            ...
            findBelongingLabelDataOfCountry(countryIndex) {
                const labelData = this.modelValue.view.foreground.countryLabels.find(label => label.ownerCountryKey == countryIndex)
            ...
            isCountryExistAtIndex(countryIndex) {
                const countryData = this.modelValue.view.foreground.countries.find(country => country.countryKey == countryIndex)
            ...
            findCountryData(countryIndex) {
                this.assertCountryExistAtIndex(countryIndex);
                return this.modelValue.view.foreground.countries.find(country => country.countryKey == countryIndex);
            },

            
            isCellExistAtIndex(cellIndex) {
                if(cellIndex in this.modelValue.state.cells) {
                    return true;
                }
                return false;
            },
            ...
            findCellData(cellIndex) {
                this.assertCellExistAtIndex(cellIndex);
                return this.modelValue.state.cells[cellIndex];
            },


            isPlayerExistAtIndex(playerIndex) {
                if(playerIndex in this.modelValue.state.players) {
                    return true;
                }
                return false;
            },
            ...
            findPlayerData(playerIndex) {
                this.assertPlayerExistAtIndex(playerIndex);
                return this.modelValue.state.players[playerIndex];
            },
        },
        created() {
            this.eventInit();
