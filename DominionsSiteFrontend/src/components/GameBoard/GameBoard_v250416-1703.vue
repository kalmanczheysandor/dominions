<template>
    ...
    <div v-if="(configuration.view !== null)">
        ...
        <div id="PlayersPanel">
            <table>
                <tr v-for="(playerItem, playerIndex) in state.players" :key="'PlayersPanel-Row-'+playerIndex">
                ...
                <g transform="translate(0,0)">
                    <g transform="scale(1.0,1.0)">
                        ...
                        <g transform="translate(0,0)">
                            <g transform="scale(1.0,1.0)">
                                <!--MERIDIAN LINES-->
                                <line v-for="(meridianItem, meridianIndex) in view.background.meridians" :key="'Background-MeridianLine-'+meridianIndex" :x1="meridianItem.x1" :y1="meridianItem.y1" :x2="meridianItem.x2" :y2="meridianItem.y2" style="stroke:#0066ff;stroke-width:1"/>
                                <!--PARALLEL LINES-->
                                <line v-for="(parallelItem, parallelIndex) in view.background.parallels" :key="'Background-ParallelsLine-'+parallelIndex" :x1="parallelItem.x1" :y1="parallelItem.y1" :x2="parallelItem.x2" :y2="parallelItem.y2" style="stroke:#0066ff;stroke-width:1"/>
                            </g>
                        </g>
                        ...
                        <g transform="translate(+250,0)">
                            <g transform="scale(0.8,0.8)">
                                <!-- CONTINENT OUTLINE -->
                                <polygon v-for="(continentItem, continentIndex) in view.foreground.continents " :key="'ContinentOutline-'+continentIndex" class="Continent" :points="continentItem.points"/>

                                <!--LINKS-->
                                <line v-for="(lineItem, lineIndex) in view.foreground.jumpLines" :key="'Line-'+lineIndex" :x1="lineItem.x1" :y1="lineItem.y1" :x2="lineItem.x2" :y2="lineItem.y2" stroke="#003d99" stroke-width="5" stroke-dasharray="5"/>

                                <!--COUNTRIES-->
                                <polygon v-for="(countryItem, countryIndex) in view.foreground.countries" :key="'Country-'+countryIndex" ffw-type="Country" :ffw-country-key="countryItem.countryKey" :class="countryItem.classes" :points="countryItem.points" :ref="'Country-' + countryItem.countryKey" @click="eventCountryIsChosen(countryItem.countryKey)"/>
                                ...
                                <!--COUNTRY LABELS-->
                                <text v-for="(countryLabelItem, countryLabelIndex) in view.foreground.countryLabels" :key="'CountryLabel-'+countryLabelIndex" ffw-type="CountryLabel" :ffw-country-key="countryLabelItem.ownerCountryKey" :x="countryLabelItem.x" :y="countryLabelItem.y" class="CountryLabel" :ref="'CountryLabel-' + countryLabelItem.labelKey">
    ...
    <div id="PendingLayer" v-if="isRecruiting">
        <div id="BackDrop"></div>
        <div class="TDialog PendingDialog">

            <div class="Body">
                <v-progress-linear color="light-blue" height="2" striped indeterminate rounded></v-progress-linear>
                <p> Waiting for {{(maxHumanPlayerCount - currentHumanPlayerCount)}}/{{maxHumanPlayerCount}} more
                    player(s) to join</p>
            </div>
            <div class="Footer">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async init(initialisationData) {
                this.settings = initialisationData.settings;
                this.configuration = initialisationData.configuration;
                this.state = initialisationData.state;
                this.statusCode = initialisationData.statusCode;

                console.log("STATE");
                console.log(initialisationData.state);

                console.log("XXXXXX Code");
                console.log(initialisationData.statusCode);
...
// alert(7)
            ...
            adjustCountryLabels() {
                for(const labelItem of this.view.foreground.countryLabels) {
            ...
            adjustCountryLabelPosition(labelItem) {
                ...
                const countryItem = this.view.foreground.countries.find(country => country.countryKey === labelItem.ownerCountryKey)
            ...
            countOccupiedCells(playerIndex) {
                ...
                Object.entries(this.state.cells).forEach((cellItem) => {
            ...
            findBelongingLabelDataOfCountry(countryIndex) {
                const labelData = this.view.foreground.countryLabels.find(label => label.ownerCountryKey == countryIndex)
            ...
            isCountryExistAtIndex(countryIndex) {
                const countryData = this.view.foreground.countries.find(country => country.countryKey == countryIndex)
            ...
            findCountryData(countryIndex) {
                this.assertCountryExistAtIndex(countryIndex);
                return this.view.foreground.countries.find(country => country.countryKey == countryIndex);
            },
            ...
            findCellData(cellIndex) {
                this.assertCellExistAtIndex(cellIndex);
                return this.state.cells[cellIndex];
            },
