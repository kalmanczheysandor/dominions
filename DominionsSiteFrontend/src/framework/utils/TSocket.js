...
class TSocket  {
    ...
    static async waitForResponse(stompClient, endpointUrl, chanelUrl, requestObj, timeout = 5000) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const subscription = stompClient.subscribe(chanelUrl, (responseObj) => {
                let contentObj = TSocket.handleSocketResponse(responseObj);
                resolve(contentObj);
    ...
    static async listenOnAChanel(stompClient, chanelUrl, callback) {
        ...
        // Subscription to a channel
        console.log("LISTEN-ON:"+chanelUrl)
        console.log("LISTEN-1")
        const subscription = stompClient.subscribe(chanelUrl, async (responseObj) => {
            console.log("LISTEN-2")
            const contentObj =  await TSocket.handleSocketResponse(responseObj);
            callback(contentObj);
        });
    }
