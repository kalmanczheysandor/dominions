package hu.kalmancheysandor.applications.dominions.apis.ai.engine.liz;
...
public class LizAiAdvancedEngine extends TDefensiveEngine implements INeuralAiEngine {

    private Map<String, INeuralNetwork> enemyNetworks;

    public LizAiAdvancedEngine() {
    ...
    protected Map<Double, Set<Integer>> buildCellRankGroups(GameState gameState, int supportedPlayerKey, Map<Integer, AiDecisionContext.Player> enemyPlayers){
        return determineConflictGroups(gameState, supportedPlayerKey, enemyPlayers);
    }

    private double[] networkModelOutput(GameState gameState,String enemyPlayerUuid,int supportedPlayerKey) {
        ...
        double[] modelOutput = enemyPlayerNeuralNetwork.calculate(networkInputData);
    ...
    protected Map<Double, Set<Integer>> determineConflictGroups(GameState gameState, int supportedPlayerKey, Map<Integer, AiDecisionContext.Player> enemyPlayers) {
        ...
        for (Map.Entry<Integer, AiDecisionContext.Player> enemyPlayerEntry : enemyPlayers.entrySet()) {
            ...
            //
            modelOutput = networkModelOutput(gameState,enemyPlayerUuid,supportedPlayerKey);
    ...
    private LizInputData generateNetworkInputDataByGameState( GameState gameState, int supportedPlayerKey) {
        ...
        // Initialise
        int boardCellCount = gameState.getCellCount();
        Map<Integer, Integer> rankMap = LizAiAdvancedEngine.createRankMapOfOccupierIndexes(gameState, supportedPlayerKey);
