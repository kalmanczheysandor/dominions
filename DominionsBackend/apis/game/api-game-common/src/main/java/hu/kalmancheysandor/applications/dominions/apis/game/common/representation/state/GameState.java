...
public class GameState {
    ...
    public void releaseAllOccupiedCellsOfPlayer(int playerIndex) {
        GameStateCell[] cells = getCells();

        for (int cellKey = 0; cellKey < cells.length; cellKey++) {

            if (cells[cellKey].getOccupierKey() == playerIndex) {
                cells[cellKey].setOccupierKey(-1);
            }
        }
    }

    public void demolishAPlayerRealm(int playerIndex) {
        if (playerIndex >= 0 && playerIndex < getCells().length) {
