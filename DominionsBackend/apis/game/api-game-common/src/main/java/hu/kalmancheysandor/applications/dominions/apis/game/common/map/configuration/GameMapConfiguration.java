...
public class GameMapConfiguration implements Serializable {
    ...
    private int getPlayerCountOfAType(GameMapPlayerType gameMapPlayerType) {
        if(players==null || players.isEmpty()) {
            return 0;
        }

        int count=0;
        for(Map.Entry<Integer, GameMapConfigurationPlayer> playerEntry: players.entrySet()) {
            if(playerEntry.getValue().getType() == gameMapPlayerType){
                count++;
            }
        }
        return count;
    ...
    public int getHumanPlayerCount() {
        return getPlayerCountOfAType(GameMapPlayerType.HUMAN);
    }

    public int getArtificialPlayerCount() {
        return getPlayerCountOfAType(GameMapPlayerType.ARTIFICIAL);
    }
