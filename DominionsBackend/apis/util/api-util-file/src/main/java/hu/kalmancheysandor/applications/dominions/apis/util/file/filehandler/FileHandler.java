package hu.kalmancheysandor.applications.dominions.apis.util.file.filehandler;

import hu.kalmancheysandor.applications.dominions.apis.util.file.filehandler.exception.*;
...
public class FileHandler {
    ...
    public static Result accessFileContent(@NotBlank String fullPath) {
        try {
    ...
    public static Result accessFileContentIfExists(@NotBlank String fullPath) {
        try {
            Path fullPathObj = Paths.get(fullPath).normalize();
            Resource resource = new UrlResource(fullPathObj.toUri());
    ...
    public static void saveBase64Image(@NotBlank String fullPath, @NotBlank String base64Image) {
        try {
    ...
    public static void updateBase64Image(@NotBlank String fullPath, @NotBlank String base64Image) {
        try {
            ...
            if(!base64Image.isEmpty()) {
                saveBase64Image(fullPath, base64Image);
            }
        } catch (IOException exp) {
            throw new GeneralFileException(fullPath);
        }
    ...
    public static void saveMultipartFile(@NotBlank String folderPath, @NotNull MultipartFile file, @NotBlank String newFilename) {
        try {
    ...
    public static void saveMultipartFile(@NotBlank String folderPath, @NotNull MultipartFile file) {
        String fileName = file.getOriginalFilename();
        saveMultipartFile(folderPath, file, fileName);
    }


    public static void updateMultipartFile(@NotBlank String folderPath, @NotNull MultipartFile file, @NotBlank String newFilename) {
        try {
            ...
            // Save as new file
            saveMultipartFile(folderPath, file, newFilename);
        } catch (TFileException e) {
            throw e;
        }
    }
    ...
    public static void updateMultipartFile(@NotBlank String folderPath, @NotNull MultipartFile file) {
        String fileName = file.getOriginalFilename();
        updateMultipartFile(folderPath, file, fileName);
    }

    public static void deleteFileIfExists(@NotBlank String fullPath) {
        try {
    ...
    public static void deleteDirectoryRecursively(@NotBlank String fullPath) {
        deleteDirectoryRecursively(fullPath,false);
    }
    public static void deleteDirectoryRecursivelyIfExists(@NotBlank String fullPath) {
        deleteDirectoryRecursively(fullPath,true);
    }


    private static void deleteDirectoryRecursively(@NotBlank String fullPath,boolean isSilent) {
        try {
            ...
            if (!Files.exists(fullPathObj)) {
                ...
                throw new DirectoryAccessException(fullPathObj.toString());
            }
    ...
    public static void createDirectoryIfNotExist(String directoryName) {
        ...
        try {
            Files.createDirectories(path); // Creates if it does not exist
        } catch (IOException e) {
            throw new DirectoryCreationException(path.toString());
        }
    }


    public static List<String> listFilesOfFolder(@NotBlank String folderPath) {
        return listFilesOfFolder(folderPath, false);
    }

    public static List<String> listFilesOfFolder(@NotBlank String folderPath, boolean isSilentMode) {
        Path folderPathObj = Paths.get(folderPath);
